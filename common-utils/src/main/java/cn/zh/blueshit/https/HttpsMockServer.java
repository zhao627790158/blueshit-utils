...
public class HttpsMockServer extends HttpsMockBase {
	...
	private static void doHttpsShakeHands(Socket s) throws Exception {
		...
		// 第二步，发送服务器证书到客户端 客户端根据字节码 还原成证书类
		...
		System.out.println("证书内容为:"
				+ CertifcateUtils.byte2hex(certificateBytes));
		...
		System.out.println("读取到的客户端的随机数为:"
				+ CertifcateUtils.byte2hex(secureBytes));
		byte secureSeed[] = decrypt(secureBytes, null);
		System.out
				.println("解密后的随机数密码为:" + CertifcateUtils.byte2hex(secureSeed));
		...
		System.out.println("客户端发送的加密消息为 : " + CertifcateUtils.byte2hex(data));
		...
		System.out.println("客户端SHA1摘要为 : "
				+ CertifcateUtils.byte2hex(clientHashBytes));
		...
		System.out.println("生成密码用于加密服务器端消息,secureRandom : "
				+ CertifcateUtils.byte2hex(secureSeed));
		...
		System.out.println("服务器端发送的机密后的消息为:"
				+ CertifcateUtils.byte2hex(serverMessage) + ",加密密码为:"
				+ CertifcateUtils.byte2hex(secureSeed));

		System.out.println("服务器端开始计算hash摘要值");
		byte serverMessageHash[] = cactHash(randomMessage.getBytes());
		System.out.println("服务器端计算的hash摘要值为 :"
				+ CertifcateUtils.byte2hex(serverMessageHash));
		SocketUtils
	...
	private static void doSocketTransport(Socket s) {
		try {
			...
			System.out.println("客户端指令内容为:"
					+ CertifcateUtils.byte2hex(clientMsg));

...
public class HttpsMockClient extends HttpsMockBase {
	...
	public static void main(String[] args) throws  Exception {
        ...
        System.out.println("服务器反馈消息:"+CertifcateUtils.byte2hex(msg));  
  ...
  //-----------------------------------------------------------------------
        ...
        System.out.println("生成的随机数为 : " + CertifcateUtils.byte2hex(seedBytes));  
        ...
        System.out.println("加密后的seed值为 :" + CertifcateUtils.byte2hex(encrptedSeed));  
        ...
        System.out.println("客户端使用SHA1计算消息摘要,加密之前的数据HASH");  
        byte hash[]=cactHash(message.getBytes());  
        System.out.println("摘要信息为:"+CertifcateUtils.byte2hex(hash));  
  ...
  //----------------------------------------------------------------------------
        ...
        System.out.println("服务器端的消息内容为 ：" + CertifcateUtils.byte2hex(serverMessage));  
  
        System.out.println("开始用之前生成的随机密码和DES算法解密消息,密码为:"+CertifcateUtils.byte2hex(seedBytes));  
        ...
        System.out.println("解密后的消息为:"+CertifcateUtils.byte2hex(decrpytedServerMsg));  
  
        int serverHashLength=in.readInt();  
        byte[] serverHash=SocketUtils.readBytes(in,serverHashLength);  
        System.out.println("开始接受服务器端的摘要消息:"+CertifcateUtils.byte2hex(serverHash));  
  
        byte[] serverHashValues=cactHash(decrpytedServerMsg);  
        System.out.println("计算服务器端发送过来的消息的摘要 : " +CertifcateUtils.byte2hex(serverHashValues));  
	...
	public static byte[] readBytes(int length) throws  Exception{  
        ...
        System.out.println("读取未解密消息:"+CertifcateUtils.byte2hex(undecrpty));  
    ...
    public static void writeBytes(byte[] data) throws  Exception{  
        ...
        System.out.println("写入加密后消息:"+CertifcateUtils.byte2hex(encrpted));  
